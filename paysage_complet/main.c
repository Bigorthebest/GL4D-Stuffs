...
void draw(void) {
  ...
  glUseProgram(_pId[0]);
  glUniform4f(glGetUniformLocation(_pId[0], "lumpos") ,0.0f, 0.0f, -3.0f, 1.0f);
  ...
  if(prog_glitch[indice_glitch] == 3){
    glUniform4f(glGetUniformLocation(_pId[0], "lambient"), 1.0f, 0.2f, 0.0f, 1.0f);
    glUniform4f(glGetUniformLocation(_pId[0], "sambient"),  0.80f, 1.0f, 0.0f, 1.0f);
    glUniform4f(glGetUniformLocation(_pId[0], "sdiffus"),  0.6f, 0.6f, 0.0f, 1.0f);
  ...
  gl4duSendMatrices();
  if(prog_glitch[indice_glitch] == 3){
    glUniform1f(glGetUniformLocation(_pId[0], "temps"), 0.0);
  }
  else{
  ...
  if(prog_glitch[indice_glitch] == 3){
    ...
    glUniform1f(glGetUniformLocation(_pId[0], "temps"), a);
  } 
  gl4dgDraw(_quadId);
   useNoiseTextures(_pId[1], 0);
   gl4dgDraw(_sphereId);
  ...
  if(t > 10){//On commence la prog après un petit temps 
    if(compt >= 30){
      indice_glitch += 1 ;
