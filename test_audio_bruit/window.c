...
static void mixCallback(void *udata, Uint8 *stream, int len) {
  ...
  double frequency = *(double *)userdata; // Récupérer la fréquence
  int16_t *buffer = (int16_t *)stream;
  int samples = len / sizeof(int16_t);

  for (int i = 0; i < samples; ++i) {
      buffer[i] = (int16_t)(sin(phase) * 30000); // Amplitude
      phase += 2.0 * M_PI * frequency / ECHANTILLONS;
      if (phase > 2.0 * M_PI) phase -= 2.0 * M_PI; // Réinitialise la phase
  }
}
